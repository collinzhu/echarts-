...
function TimeInboundSpam(props: ITimeInboundSpam) {
  ...
  const body = sort.map((data) => {
    return [demo(data.name), data.value];
  });
  console.log("body", body);
  const data = [head, ...body];
  const Option = {
    title: {
      text: "Execution count of timebound data",
      subtext: "Demonstrates how timebound data could be represented",
    },
    dataZoom: [
      {
        id: "dataZoomX",
  ...
  return (
    <div>
      <ReactEcharts option={Option} style={{ height: 500 }} theme="theme" />;
    </div>
  );
}
